		...
		private void ProcessItems(IAbstractMarkupDataContainer sourceContainer, ref Stack<IAbstractMarkupDataContainer> containers)
		{
			foreach (var item in sourceContainer)
			{
				...
				if (item is ITagPair tagPair)
				{
					...
					var newTagPair = tagPair.Clone() as ITagPair;
					newTagPair.Clear();
					//newTagPair.ClearSubSegmentReferences();
				...
				else if (item is ICommentMarker commentMarker)
				{
					var newCommentMarker = commentMarker.Clone() as ICommentMarker;
					newCommentMarker.Clear();

					container.Add(newCommentMarker);
					containers.Push(newCommentMarker);

					ProcessItems(commentMarker, ref containers);
