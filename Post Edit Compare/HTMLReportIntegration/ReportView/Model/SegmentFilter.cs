...
namespace Sdl.Community.PostEdit.Versions.HTMLReportIntegration.ReportView.Model
{
    public class SegmentFilter
    {
        ...
        public int FuzzyPercentage { get; set; }
        ...
        public bool IsMatch(ReportSegment segment)
        {
            ...
            if (Statuses != 0)
            {
                ...
                if (!Statuses.HasFlag(confirmationStatus)) return false;
            }

            if (MatchTypes !=0)
            {
                if (string.IsNullOrWhiteSpace(segment.MatchType)) return false;
                ...
                if (!Enum.TryParse<MatchTypes>(segment.MatchType, out var matchType))
                    ...
                    else
                    {
                        ...
                        if (FuzzyPercentage != fuzzyPercentage) return false;
                    }
