...
namespace InterpretBank.SettingsService.ViewModel
{
    public class GlossarySetupViewModel : ViewModelBase.ViewModel
    {
        ...
        {
            if (!UserInteractionService.GetFilePath(out var filepath)) return;
            var importTerms = ExchangeService.ImportTerms(filepath).ToList();

            //InterpretBankDataContext.AddFullTerms(importTerms, SelectedGlossary.Id);
        }
