...
namespace ChangeScalingBehavior
{
	public partial class HighDPIChange : Form
	{
		...
		private void HighDPIChange_Load(object sender, EventArgs e)
		{
			...
			AddBetaSuffixToPublicVersionName();
		...
		private void AddBetaSuffixToPublicVersionName()
		{
			foreach (var installedStudioVersion in _installedStudioVersions)
			{
				if (string.Compare(installedStudioVersion.Edition, "Beta", StringComparison.InvariantCultureIgnoreCase) == 0)
				{
					installedStudioVersion.PublicVersion += $" {installedStudioVersion.Edition}";
				}
			}
		}
