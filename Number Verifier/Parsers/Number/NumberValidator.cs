...
namespace Sdl.Community.NumberVerifier.Parsers.Number
{
	public class NumberValidator
	{
		...
		//private static NumberList GetNumberParts2(List<string> thousandSeparators, List<string> decimalSeparators, List<MatchArray> textMatches)
		//{
		//	var matchesNormalized = GetNormalizedTexts(thousandSeparators, decimalSeparators, textMatches);
		//	var normalizedParts = new List<string>();
		//	var initialParts = new List<string>();
		//	for (var i = 0; i < textMatches.Count; i++)
		//	{
		//		var normalizedMatch = matchesNormalized[i];
		//		var currentNormalizedParts = normalizedMatch.Split("*".ToCharArray()).ToList();
		//		currentNormalizedParts.RemoveAll(string.IsNullOrWhiteSpace);
		//		var currentInitialParts = textMatches[i]["Parts"].Select(stm => stm.Value).ToList();

		//		var sign = textMatches[i]["Sign"][0].Value;
		//		if (sign is not null) currentInitialParts[0] = currentInitialParts[0].Insert(0, sign);

		//		normalizedParts.AddRange(currentNormalizedParts);
		//		initialParts.AddRange(currentInitialParts);
		//	}

		//	return new NumberList(initialParts.Select(n => n.Normalize(NormalizationForm.FormKC)).ToList(),
		//		normalizedParts.Select(n => n.Normalize(NormalizationForm.FormKC)).ToList());
		//}

		//private static TextAreas GetNumberParts(List<string> thousandSeparators, List<string> decimalSeparators, List<Match> textMatches)
		//{
		//	var textAreas = new TextAreas();

		//	var matchesNormalized = GetNormalizedTexts(thousandSeparators, decimalSeparators, textMatches);
		//	for (var i = 0; i < textMatches.Count; i++)
		//	{
		//		var normalizedMatch = matchesNormalized[i];
		//		textAreas[textMatches[i].Value] = normalizedMatch.Item2;
		//	}

		//	return textAreas;
		//}

		//private static List<(string, List<string>)> GetNormalizedTexts(List<string> thousandSeparators, List<string> decimalSeparators, List<Match> matchArray)
		//{
		//	var textsNormalized = new List<(string, List<string>)>();
		//	foreach (var match in matchArray)
		//	{
		//		var separators = match.Groups["Separators"].Captures.Cast<Capture>().Select(sep => sep.Value).ToList();
		//		var sourceNumberText = match.Value;

		//		//we don't have to worry about undefined separators since when we get here, text is already split by undefined separators
		//		var sign = match.Groups["Sign"].Captures.Cast<Capture>().FirstOrDefault();
		//		if (sign is not null) sourceNumberText = sourceNumberText.Replace(sign.Value, "*s");

		//		var index = 0;
		//		foreach (var separator in separators.Distinct().ToList())
		//		{
		//			var normalizedSeparator = "na";
		//			if (thousandSeparators.Contains(separator)) normalizedSeparator = "*t";
		//			if (decimalSeparators.Contains(separator))
		//				normalizedSeparator = normalizedSeparator == "na" ? "*d" : $"*u<{index++}>";

		//			ReplaceInList(separators, separator, normalizedSeparator);
		//			sourceNumberText = sourceNumberText.Replace(separator, normalizedSeparator);
		//		}

		//		textsNormalized.Add((sourceNumberText, separators));
		//	}

		//	return textsNormalized;
		//}
	}
}