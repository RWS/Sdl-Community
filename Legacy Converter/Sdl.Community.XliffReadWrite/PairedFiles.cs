...
namespace Sdl.Community.XliffReadWrite
{
    internal class PairedFiles
    {
        ...
        public PairedFiles(string originalDirectory, string updatedDirectory, string[] filters, bool processSubFolders)
        {
            ...
            foreach (var filePathOriginal in filePathsOriginal)
            {
                var pairedFile = new PairedFile
                {
                    OriginalFilePath = new FileInfo(filePathOriginal),
                ...
                var originalPathCompare = filePathOriginal.Replace(originalDirectory, string.Empty);
                ...
                foreach (var filePathUpdated in filePathsUpdated)
                {
                    ...
                    if (string.Compare(originalPathCompare, updatedPathCompare, StringComparison.OrdinalIgnoreCase) != 0) continue;
                ...
                if (!found)
                {
                    ...
                    pairedFile.Message = string.Format("Unable to locate the associated file in th updated folder {0}", originalPathCompare);
                }
