...
namespace Sdl.Community.XliffToLegacyConverter.Core.Word
{
    internal class Processor
    {
                    ...
                    while (fnd.Found)
                    {
                        if (includeLegacyStructure)
                        {
                            ...
                            if (mRegSeg.Success)
                            {
                                ...
                                if (isLocked && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportLocked)
                                    importTranslation = false;
                                else if (!isLocked && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportUnLocked)
                                    importTranslation = false;
                                else if (string.Compare(matchTypeId, "Perfect Match", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportPerfectMatch)
                                    importTranslation = false;
                                else if (string.Compare(matchTypeId, "Context Match", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportContextMatch)
                                    importTranslation = false;
                                else if (string.Compare(matchTypeId, "Exact Match", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportExactMatch)
                                    importTranslation = false;
                                else if (string.Compare(matchTypeId, "Fuzzy Match", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportFuzzyMatch)
                                    importTranslation = false;
                                else if (string.Compare(matchTypeId, "No Match", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportNoMatch)
                                    importTranslation = false;

                                if (string.Compare(segmentStatus, "Not Translated", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportNotTranslated)
                                    importTranslation = false;
                                else if (string.Compare(segmentStatus, "Draft", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportDraft)
                                    importTranslation = false;
                                else if (string.Compare(segmentStatus, "Translated", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslated)
                                    importTranslation = false;
                                else if (string.Compare(segmentStatus, "Translation Approved", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslationApproved)
                                    importTranslation = false;
                                else if (string.Compare(segmentStatus, "Translation Rejected", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslationRejected)
                                    importTranslation = false;
                                else if (string.Compare(segmentStatus, "Sign-off Rejected", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportSignOffRejected)
                                    importTranslation = false;
                                else if (string.Compare(segmentStatus, "Sign-off", StringComparison.OrdinalIgnoreCase) == 0 && Sdl.Community.XliffReadWrite.Processor.ProcessorSettings.DoNotImportSignOff)
                                    importTranslation = false;
                                    ...
                                    if (mcRGetSegs.Count > 0)
                                    {
                                        ...
                                        foreach (Match mRGetSegs in mcRGetSegs)
                                        {
                                            ...
                                            if (percentage != string.Empty)
                                            {
                                                try
                                                {
                                                    segmentPair.TranslationOrigin.MatchPercentage = Convert.ToInt32(percentage);
                                                }
                                                catch
                                                {
                                                    // ignored
                                            ...
                                            #region  |  source  |
                                            var mcRegInternalSource = regInternal.Matches(sourceText);
                                            var lastIndexSource = 0;
                                            foreach (Match mRegInternal in mcRegInternalSource)
                                            {
                                                var matchIndex = mRegInternal.Index;

                                                if (matchIndex > lastIndexSource)
                                                {
                                                    var text = sourceText.Substring(lastIndexSource, (matchIndex - lastIndexSource)).Replace("\v", "\n");
                                                    sourceSections.Add(new SegmentSection(SegmentSection.ContentType.Text, "", text));
                                                }
                                                ...
                                                foreach (var tag in tags)
                                                {
                                                    switch (tag.Type)
                                                    {
                                                        case TagUnit.TagUnitType.IsPlaceholder:
                                                            {
                                                                sourceSections.Add(new SegmentSection(SegmentSection.ContentType.Placeholder, tag.Id, tag.Content));
                                                            } break;
                                                        case TagUnit.TagUnitType.IsTag:
                                                            {
                                                                switch (tag.State)
                                                                {
                                                                    case TagUnit.TagUnitState.IsOpening:
                                                                        {
                                                                            sourceSections.Add(new SegmentSection(SegmentSection.ContentType.Tag, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                                            break;
                                                                        }
                                                                    case TagUnit.TagUnitState.IsClosing:
                                                                        {
                                                                            sourceSections.Add(new SegmentSection(SegmentSection.ContentType.TagClosing, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                                            break;
                                                                        }
                                                                    case TagUnit.TagUnitState.IsEmpty:
                                                                        {
                                                                            sourceSections.Add(new SegmentSection(SegmentSection.ContentType.Placeholder, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                                            break;
                                                                        }
                                                                }
                                                            } break;
                                                        case TagUnit.TagUnitType.IsLockedContent:
                                                            {
                                                                sourceSections.Add(new SegmentSection(SegmentSection.ContentType.LockedContent, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                            } break;
                                                    }
                                                }
                                                #endregion
                                            ...
                                            if (lastIndexSource < sourceText.Length)
                                            {
                                                var text = sourceText.Substring(lastIndexSource).Replace("\v", "\n");
                                                sourceSections.Add(new SegmentSection(SegmentSection.ContentType.Text, "", text));
                                            }
                                            ...
                                            #region  |  target  |
                                            var mcRegInternalTarget = regInternal.Matches(targetText);
                                            var lastIndexTarget = 0;
                                            foreach (Match mRegInternal in mcRegInternalTarget)
                                            {
                                                var matchIndex = mRegInternal.Index;

                                                if (matchIndex > lastIndexTarget)
                                                {
                                                    var text = targetText.Substring(lastIndexTarget, (matchIndex - lastIndexTarget)).Replace("\v", "\n");
                                                    targetSections.Add(new SegmentSection(SegmentSection.ContentType.Text, "", text));
                                                }
                                                ...
                                                foreach (var tag in tags)
                                                {
                                                    switch (tag.Type)
                                                    {
                                                        case TagUnit.TagUnitType.IsPlaceholder:
                                                            {
                                                                targetSections.Add(new SegmentSection(SegmentSection.ContentType.Placeholder, tag.Id, tag.Content));
                                                                break;
                                                            }
                                                        case TagUnit.TagUnitType.IsTag:
                                                            {
                                                                switch (tag.State)
                                                                {
                                                                    case TagUnit.TagUnitState.IsOpening:
                                                                        {
                                                                            targetSections.Add(new SegmentSection(SegmentSection.ContentType.Tag, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                                            break;
                                                                        }
                                                                    case TagUnit.TagUnitState.IsClosing:
                                                                        {
                                                                            targetSections.Add(new SegmentSection(SegmentSection.ContentType.TagClosing, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                                            break;
                                                                        }
                                                                    case TagUnit.TagUnitState.IsEmpty:
                                                                        {
                                                                            targetSections.Add(new SegmentSection(SegmentSection.ContentType.Placeholder, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                                            break;
                                                                        }
                                                                }
                                                                break;
                                                        ...
                                                        case TagUnit.TagUnitType.IsLockedContent:
                                                            {
                                                                targetSections.Add(new SegmentSection(SegmentSection.ContentType.LockedContent, tag.Id, tag.Content.Replace("\v", "\r\n")));
                                                            } break;
                                                    }
                                                }
                                                #endregion
                                            ...
                                            if (lastIndexTarget < targetText.Length)
                                            {
                                                var text = targetText.Substring(lastIndexTarget).Replace("\v", "\n");
                                                targetSections.Add(new SegmentSection(SegmentSection.ContentType.Text, "", text));
                                            }
                                            #endregion
                                        }
                                        ...
                                        var iTagIdSource = 10000;
                                        for (var ix = 0; ix < sourceSections.Count; ix++)
                                        {
                                            var section = sourceSections[ix];
                                            switch (section.Type)
                                            {
                                                case SegmentSection.ContentType.Tag:
                                                    {
                                                        iTagIdSource++;
                                                        var tagId = "xpt" + iTagIdSource;
                                                        var tagName = Core.Processor.GetStartTagName(section.Content, ref tagId);

                                                        currentTagListSource.Add(new TagUnit(tagId, tagName, section.Content, TagUnit.TagUnitState.IsOpening, TagUnit.TagUnitType.IsTag));
                                                        section.Content = Core.Processor.GetContentTypeToMarkup(section.Type, section.Content, currentTagListSource[currentTagListSource.Count - 1].Id);
                                                    }
                                                    break;
                                                case SegmentSection.ContentType.TagClosing:
                                                    var closingTagName = Core.Processor.GetEndTagName(section.Content);
                                                    if (currentTagListSource.Count > 0)
                                                    {
                                                        if (string.Compare(currentTagListSource[currentTagListSource.Count - 1].Name, closingTagName, StringComparison.OrdinalIgnoreCase) == 0)
                                                        {
                                                            section.Content = Core.Processor.GetContentTypeToMarkup(section.Type, section.Content, currentTagListSource[currentTagListSource.Count - 1].Id);

                                                            currentTagListSource.RemoveAt(currentTagListSource.Count - 1);
                                                        }
                                                        else
                                                        {
                                            ...
                                            sourceSections[ix] = section;
                                        }
                                        #endregion

                                        #region  |  tags target |
                                        var currentTagListTarget = new List<TagUnit>();
                                        var iTagIdTarget = 10000;
                                        for (var ix = 0; ix < targetSections.Count; ix++)
                                        {
                                            ...
                                            switch (section.Type)
                                            {
                                                case SegmentSection.ContentType.Tag:
                                                    {
                                                        iTagIdTarget++;
                                                        var tagId = "xpt" + iTagIdTarget;
                                                        var tagName = Core.Processor.GetStartTagName(section.Content, ref tagId);

                                                        currentTagListTarget.Add(new TagUnit(tagId, tagName, section.Content, TagUnit.TagUnitState.IsOpening, TagUnit.TagUnitType.IsTag));
                                                        section.Content = Core.Processor.GetContentTypeToMarkup(section.Type, section.Content, currentTagListTarget[currentTagListTarget.Count - 1].Id);
                                                    }
                                                    break;
                                                case SegmentSection.ContentType.TagClosing:
                                                    var closingTagName = Core.Processor.GetEndTagName(section.Content);
                                                    if (currentTagListTarget.Count > 0)
                                                    {
                                                        if (string.Compare(currentTagListTarget[currentTagListTarget.Count - 1].Name, closingTagName, StringComparison.OrdinalIgnoreCase) == 0)
                                                        {
                                                            section.Content = Core.Processor.GetContentTypeToMarkup(section.Type, section.Content, currentTagListTarget[currentTagListTarget.Count - 1].Id);

                                                            currentTagListTarget.RemoveAt(currentTagListTarget.Count - 1);
                                                        }
                                                        else
                                                        {
                                                            section.Content = Core.Processor.GetContentTypeToMarkup(section.Type, section.Content, string.Empty);
                        ...
                        else
                        {
                            ...
                            if (mRGetSegs.Success)
                            {
                                ...
                                for (var ix = 0; ix < targetSections.Count; ix++)
                                {
                                    ...
                                    else if (section.Type == SegmentSection.ContentType.TagClosing)
                                    {
                                        var closingTagName = Core.Processor.GetEndTagName(section.Content);
                                        if (currentTagListTarget.Count > 0)
                                        {
        ...
        {
            if (StyleTw4WinExternal != null)
                StyleTw4WinExternal.Delete();

            if (StyleTw4WinInternal != null)
                StyleTw4WinInternal.Delete();

            if (StyleTw4WinMark != null)
