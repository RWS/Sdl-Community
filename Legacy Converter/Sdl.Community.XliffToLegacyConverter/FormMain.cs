...
namespace Sdl.Community.XliffToLegacyConverter
{
	public partial class FormMain : Form
	{
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
			...
			comboBox_OuputFormat.SelectedIndex = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsConverToFormat;

			checkBox_includeLegacyStructure.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsIncludeLegacyStructure;
			checkBox_reverseLanguageDirection.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsReverseLanguageDirection;
			checkBox_viewReportWhenProcessingFinished.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsViewReportWhenProcessingFinished;

			checkBox_createBAKfile.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsCreateBaKfile;
			checkBox_excludeTags.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsExcludeTags;
		...
		private void FormMain_FormClosing(object sender, FormClosingEventArgs e)
		{
			...
			XliffReadWrite.Processor.ProcessorSettings.CreateBackupOfSdlXliffFile = checkBox_createBAKfile.Checked;

			XliffReadWrite.Processor.SaveSettings();
		}
		...
		private void RunExport()
		{
			...
			try
			{
				...
				var sdlXliffReader = new XliffReadWrite.Processor();
				...
				try
				{
					...
					foreach (ListViewItem itm in listView_export.Items)
					{
						...
						XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal = itm.Text;
						XliffReadWrite.Processor.ProcessorSettings.FilePathUpdated = string.Empty;
						...
						if (comboBox_OuputFormat.SelectedIndex == 3) //export to TMX
							XliffReadWrite.Processor.ProcessorSettings.IgnoreEmptyTranslations = true;
						...
						try
						{
							...
							{
								if (XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal.IndexOf("\\", StringComparison.Ordinal) > -1)
								{
									var path = XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal.Trim().Substring(0, XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal.Trim().LastIndexOf("\\", StringComparison.Ordinal));
							...
							case 0:
								{
									...
									converter.WriteRtf(itm.Text + ".rtf", XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal, fileParagraphUnits, sdlXliffReader.SourceLanguageCultureInfo, sdlXliffReader.TargetLanguageCultureInfo, false, checkBox_includeLegacyStructure.Checked);
								}
								break;
							case 1:
								{
									_report.Append("Converted file\t: " + itm.Text + ".docx" + "\r\n\r\n");
									converter.WriteRtf(itm.Text + ".rtf", XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal, fileParagraphUnits, sdlXliffReader.SourceLanguageCultureInfo, sdlXliffReader.TargetLanguageCultureInfo, true, checkBox_includeLegacyStructure.Checked);
								}
								break;
							case 2:
								{
									_report.Append("Converted file\t: " + itm.Text + ".ttx" + "\r\n\r\n");
									converter.WriteTtx(itm.Text + ".ttx", XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal, fileParagraphUnits, sdlXliffReader.SourceLanguageCultureInfo, sdlXliffReader.TargetLanguageCultureInfo);
								}
								break;
							case 3:
								{
									_report.Append("Converted file\t: " + itm.Text + ".tmx" + "\r\n\r\n");
									converter.WriteTmx(itm.Text + ".tmx", XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal, fileParagraphUnits, sdlXliffReader.SourceLanguageCultureInfo, sdlXliffReader.TargetLanguageCultureInfo, checkBox_excludeTags.Checked, checkBox_reverseLanguageDirection.Checked);
								}
								break;
						}
		...
		private void RunImport()
		{
			...
			{
				_report = new StringBuilder();
				var sdlXliffWriter = new XliffReadWrite.Processor();
				...
				try
				{
					...
					foreach (ListViewItem itm in listView_import.Items)
					{
						...
						XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal = Path.Combine(itm.Tag.ToString(), itm.SubItems[1].Text);

						if (!File.Exists(XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal))
							throw new Exception("Unable to locate the associated sdlxliff file for the bilingual file\r\n" + itm.Text);



						var sdlXliffReader = new XliffReadWrite.Processor();
						XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal = XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal;
						XliffReadWrite.Processor.ProcessorSettings.FilePathUpdated = string.Empty;
							...
							foreach (var fileParagraphUnitOriginal in fileParagraphUnitsOriginal)
							{
								foreach (var paragraphUnitOriginal in fileParagraphUnitOriginal.Value)
								{
									...
									segmentKeysNotFound.AddRange(from segmentPairNew in paragraphUnitNew.Value.SegmentPairs
																 ...
																 where !foundSegmentKey
																 select segmentPairNew.Id);
									break;
								}
							}
						...
						XliffReadWrite.Processor.ProcessorSettings.FilePathUpdated = XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal + "_updated.sdlxliff";
						...
						sdlXliffWriter.TargetLanguageCultureInfo = GetTargetCultureInfo(XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal, sdlXliffWriter.SourceLanguageCultureInfo, sdlXliffWriter.TargetLanguageCultureInfo);
						...
						if (XliffReadWrite.Processor.ProcessorSettings.ReportDifferencesInTagsPlaceables)
						{
						...
						if (XliffReadWrite.Processor.ProcessorSettings.CreateBackupOfSdlXliffFile)
						{
							sbReportBody.Append("Created backup of the original file\r\n" + XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal + ".bak" + "\r\n\r\n");
						}

						sbReportBody.Append("".PadRight(90, '-') + "\r\n\r\n");

						processingFilesTmp.Add(XliffReadWrite.Processor.ProcessorSettings.FilePathOriginal, XliffReadWrite.Processor.ProcessorSettings.FilePathUpdated);
					}
				...
				{
					if (XliffReadWrite.Processor.ProcessorSettings.UndoAllChangesOnException)
					{
		...
		private void CreateReport()
		{
			...
			if (checkBox_viewReportWhenProcessingFinished.Checked)
				Process.Start(Path.Combine(textBox_reportDirectory.Text, textBox_reportFileName.Text));
		}
		...
		private void LoadSettings()
		{
			var settings = new FormSettings
			{
				...
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.CopySourceToTargetEmptyTranslations
				},
				checkBox_ReportDifferencesInTags =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.ReportDifferencesInTagsPlaceables
				},
				checkBox_ignoreEmptyTranslations =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.IgnoreEmptyTranslations
				},
				checkBox_UndoChangesToAllFileOnException =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.UndoAllChangesOnException
				},
				checkBox_pm_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportPerfectMatch
				},
				checkBox_cm_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportContextMatch
				},
				checkBox_em_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportExactMatch
				},
				checkBox_fm_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportFuzzyMatch
				},
				checkBox_nm_doNotExport = { Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportNoMatch },
				checkBox_locked_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportLocked
				},
				checkBox_unlocked_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportUnLocked
				},
				checkBox_notTranslated_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportNotTranslated
				},
				checkBox_draft_doNotExport = { Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportDraft },
				checkBox_translated_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportTranslated
				},
				checkBox_translationApproved_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportTranslationApproved
				},
				checkBox_translationRejected_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportTranslationRejected
				},
				checkBox_signOffRejected_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportSignOffRejected
				},
				checkBox_signOff_doNotExport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotExportSignOff
				},
				checkBox_pm_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportPerfectMatch
				},
				checkBox_cm_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportContextMatch
				},
				checkBox_em_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportExactMatch
				},
				checkBox_fm_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportFuzzyMatch
				},
				checkBox_nm_doNotImport = { Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportNoMatch },
				checkBox_locked_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportLocked
				},
				checkBox_unlocked_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportUnLocked
				},
				checkBox_notTranslated_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportNotTranslated
				},
				checkBox_draft_doNotImport = { Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportDraft },
				checkBox_translated_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslated
				},
				checkBox_translationApproved_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslationApproved
				},
				checkBox_translationRejected_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslationRejected
				},
				checkBox_signOffRejected_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportSignOffRejected
				},
				checkBox_signOff_doNotImport =
				{
					Checked = XliffReadWrite.Processor.ProcessorSettings.DoNotImportSignOff
				},
				comboBox_UpdatedStatus =
				{
					SelectedItem =
					XliffReadWrite.Processor.ProcessorSettings.ChangedTranslationStatus != string.Empty
						? Processor.GetVisualSegmentStatus(
							XliffReadWrite.Processor.ProcessorSettings.ChangedTranslationStatus)
						: "Don't Change"
				},
				comboBox_NotUpdatedStatus =
				{
					SelectedItem =
					XliffReadWrite.Processor.ProcessorSettings.NotChangedTranslationStatus != string.Empty
						? Processor.GetVisualSegmentStatus(
							XliffReadWrite.Processor.ProcessorSettings.NotChangedTranslationStatus)
						: "Don't Change"
				},
				comboBox_NotImportedStatus =
				{
					SelectedItem =
					XliffReadWrite.Processor.ProcessorSettings.NotImportedTranslationStatus != string.Empty
						? Processor.GetVisualSegmentStatus(
							XliffReadWrite.Processor.ProcessorSettings.NotImportedTranslationStatus)
						: "Don't Change"
				}
			};
			...
			settings.comboBox_OuputFormat.SelectedIndex = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsConverToFormat;

			settings.checkBox_includeLegacyStructure.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsIncludeLegacyStructure;
			settings.checkBox_reverseLanguageDirection.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsReverseLanguageDirection;
			settings.checkBox_viewReportWhenProcessingFinished.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsViewReportWhenProcessingFinished;
			settings.checkBox_createBAKfile.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsCreateBaKfile;
			settings.checkBox_excludeTags.Checked = XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsExcludeTags;
			...
			XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsConverToFormat = settings.comboBox_OuputFormat.SelectedIndex;

			XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsIncludeLegacyStructure = settings.checkBox_includeLegacyStructure.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsReverseLanguageDirection = settings.checkBox_reverseLanguageDirection.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsViewReportWhenProcessingFinished = settings.checkBox_viewReportWhenProcessingFinished.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsCreateBaKfile = settings.checkBox_createBAKfile.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DefaultSettingsExcludeTags = settings.checkBox_excludeTags.Checked;

			XliffReadWrite.Processor.ProcessorSettings.IgnoreEmptyTranslations = settings.checkBox_ignoreEmptyTranslations.Checked;

			XliffReadWrite.Processor.ProcessorSettings.CopySourceToTargetEmptyTranslations = settings.checkBox_copySourceToTargetEmptyTranslations.Checked;

			XliffReadWrite.Processor.ProcessorSettings.ReportDifferencesInTagsPlaceables = settings.checkBox_ReportDifferencesInTags.Checked;
			XliffReadWrite.Processor.ProcessorSettings.UndoAllChangesOnException = settings.checkBox_UndoChangesToAllFileOnException.Checked;

			XliffReadWrite.Processor.ProcessorSettings.DoNotExportPerfectMatch = settings.checkBox_pm_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportContextMatch = settings.checkBox_cm_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportExactMatch = settings.checkBox_em_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportFuzzyMatch = settings.checkBox_fm_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportNoMatch = settings.checkBox_nm_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportLocked = settings.checkBox_locked_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportUnLocked = settings.checkBox_unlocked_doNotExport.Checked;


			XliffReadWrite.Processor.ProcessorSettings.DoNotExportNotTranslated = settings.checkBox_notTranslated_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportDraft = settings.checkBox_draft_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportTranslated = settings.checkBox_translated_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportTranslationApproved = settings.checkBox_translationApproved_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportTranslationRejected = settings.checkBox_translationRejected_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportSignOffRejected = settings.checkBox_signOffRejected_doNotExport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotExportSignOff = settings.checkBox_signOff_doNotExport.Checked;



			XliffReadWrite.Processor.ProcessorSettings.DoNotImportPerfectMatch = settings.checkBox_pm_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportContextMatch = settings.checkBox_cm_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportExactMatch = settings.checkBox_em_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportFuzzyMatch = settings.checkBox_fm_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportNoMatch = settings.checkBox_nm_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportLocked = settings.checkBox_locked_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportUnLocked = settings.checkBox_unlocked_doNotImport.Checked;

			XliffReadWrite.Processor.ProcessorSettings.DoNotImportNotTranslated = settings.checkBox_notTranslated_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportDraft = settings.checkBox_draft_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslated = settings.checkBox_translated_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslationApproved = settings.checkBox_translationApproved_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportTranslationRejected = settings.checkBox_translationRejected_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportSignOffRejected = settings.checkBox_signOffRejected_doNotImport.Checked;
			XliffReadWrite.Processor.ProcessorSettings.DoNotImportSignOff = settings.checkBox_signOff_doNotImport.Checked;



			XliffReadWrite.Processor.ProcessorSettings.ChangedTranslationStatus = string.Compare(settings.comboBox_UpdatedStatus.SelectedItem.ToString(), "Don't Change", StringComparison.OrdinalIgnoreCase) == 0
				? string.Empty
				: Processor.GetSegmentStatusFromVisual(settings.comboBox_UpdatedStatus.SelectedItem.ToString());
			XliffReadWrite.Processor.ProcessorSettings.NotChangedTranslationStatus = string.Compare(settings.comboBox_NotUpdatedStatus.SelectedItem.ToString(), "Don't Change", StringComparison.OrdinalIgnoreCase) == 0
				? string.Empty
				: Processor.GetSegmentStatusFromVisual(settings.comboBox_NotUpdatedStatus.SelectedItem.ToString());
			XliffReadWrite.Processor.ProcessorSettings.NotImportedTranslationStatus = string.Compare(settings.comboBox_NotImportedStatus.SelectedItem.ToString(), "Don't Change", StringComparison.OrdinalIgnoreCase) == 0
				? string.Empty
		...
		private void AddFilesToImportListView(List<FileInfo> files)
		{
			try
			{
				foreach (var file in files)
				{
					var foundInList = (from ListViewItem itm in listView_import.Items
									   let path = itm.Tag.ToString()
