...
namespace GoogleTranslatorProvider.Models
{
	...
	public class TagPlacer
	{
		...
		private Dictionary<string, GTPTag> _dict;
		...
		/// </summary>
		private Dictionary<string, GTPTag> GetSourceTagsDict()
		{
			...
			_dict = new Dictionary<string, GTPTag>();
			...
			try
			{
				for (var i = 0; i < _sourceSegment.Elements.Count; i++)
				{
					...
					{
						var theTag = new GTPTag((Tag)_sourceSegment.Elements[i].Duplicate());
