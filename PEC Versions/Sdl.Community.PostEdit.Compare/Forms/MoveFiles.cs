    ...
    public partial class MoveFiles : Form
    {
        ...
        private void UpdatedObjects()
        {
            if (radioButton_left_side.Checked)
            {
                ...
                label_scope_message.Text = "Move selected files from the left to the right side and "
                    + (checkBox_overwirte_existing_files.Checked ? "overwrite" : "do not overwrite") + " files if they already exist!";
            }
            else
            {
                label_scope_message.Text = "Move selected files from the right to the left side and "
                 + (checkBox_overwirte_existing_files.Checked ? "overwrite" : "do not overwrite") + " files if they already exist!";
