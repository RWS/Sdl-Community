...
public class EdgeCredentialsViewModel : BaseViewModel, ICredentialsViewModel
{
    ...
    private async Task<bool> Authenticate(EdgeCredentials edgeCredentials)
    {
        ...
        if (IsApiKeySelected)
        {
            ...
            success = await EdgeService.VerifyAPI(edgeCredentials, TranslationOptions);
        }
        else if (IsCredentialsSelected)
        {
            ...
            success = await EdgeService.AuthenticateUser(edgeCredentials, TranslationOptions);
        }
        else
        {
            StartLoginProcess?.Invoke(this, new LoginEventArgs(PluginResources.Loading_Edge_ConnectingAuth0));
            success = AuthenticateWithAuth0(edgeCredentials);
        }

        if (success)
        {
            StartLoginProcess?.Invoke(this, new LoginEventArgs("Gettings permissions..."));
            await EdgeService.SetUserPermisions(TranslationOptions.AccessToken);
        }
