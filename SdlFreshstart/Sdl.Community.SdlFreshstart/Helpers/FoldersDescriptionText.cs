namespace Sdl.Community.SdlFreshstart.Helpers
{
	...
	{
		public static string ProjectsXml => "This option will only remove the projects.xml file, it will not physically remove your projects from your Documents folder.  It will remove them from Studio, so if you delete this file and need to put some or all of the projects back, you will have to add the projects back one at a time by using the Open Project command in Studio.  The projects.xml contains the following information:" +
		...
		public static string ProjectsTemplates => "This is the default location for your project templates.  If you used this location for custom templates then consider backing them up so you can easily replace them.  The default template will be replaced when you restart Studio but your custom templates will not." +
				"\n ";

		public static string AppDataRoamingMajor => "This location contains the sdlplugins you have installed from the SDL AppStore that are intended to be used by one user and should be available to that user on any machine they log into under that same userprofile.  The plugins folder contains two folders:" +
		...
		public static string ProjectApi => "This folder contains one file, Sdl.ProjectApi.xml, which contains information relating to the location of local project folders. This is only the default since today Studio can be configured by the user to store projects wherever you like through the use of the project templates.  So unless you manually edited this file it seems unlikely that deleting it will cause any cause for concern.";
		...
		public static string ProgramData => "This location contains the sdlplugins you have installed from the SDL AppStore that are going to be shared across multiple users, usually in an Enterprise environment to avoid having to maintain multiple copies of the same files on every machine." +
				"\n The plugins folder contains two folders:" +
				"\n - Packages: this is where the sdlplugin files are saved when they are installed" +
				"\n - Unpacked: sdlplugin files are essentially zip files and the unzipped versions and unpacked in this folder when Studio starts" +
				"\n If you want to replace all your plugins quickly then back up the content of your 'Packages' folder first so you only have to copy them back afterwards.  You don't need to, and in fact should not, back up the 'Unpacked' folder.";

		public static string ProgramDataFull => "Settings.xml: It stores information about your MultiTerm server connections.  Deleting it might help troubleshoot server connectivity, but you may need to recreate your logins to MultiTerm servers you may be using.";

		public static string ProgramDataVersionNumber => "This folder contains two folders:" +
