//-----------------------------------------------------------------------
// <copyright file="TagAligner.cs" company="SDL plc">
...
namespace Sdl.Sdk.SdlxTmTranslationProvider.Lingua
{
    ...
    {
        public static TagAssociations AlignPairedTags(IList<Token> sourceTokens,
        ...
        public static TagPairs FindPairedTags(IList<Token> tokens)
        {
            ...
            {
                TagToken st = tokens[stp] as TagToken;
                if (st != null && st.Tag.Type == TagType.Start)
                {
                    // find the end tag, which is supposed to follow
                    int etp = 0;
                    for (etp = stp + 1; etp < tokens.Count; ++etp)
                    {
                        TagToken et = tokens[etp] as TagToken;
                        if (et != null && et.Tag.Type == TagType.End && et.Tag.Anchor == st.Tag.Anchor)
                        {
                            if (result == null)
                                result = new TagPairs();
