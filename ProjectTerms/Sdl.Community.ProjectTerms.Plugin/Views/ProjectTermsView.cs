...
namespace Sdl.Community.ProjectTerms.Plugin.Views
{
	public partial class ProjectTermsView : Form
	{
		...
		public Filters ExtractFilters()
		{
			...
			// To get already saved term occurrences and lengths
			var _filters = viewModel.GetTermFilters();			
			numericUpDownTermsOccurrences.Text =_filters.Occurrences.ToString();
			numericUpDownTermsLength.Text = _filters.Length.ToString();
			return new Filters()
			{
				Blacklist = ExtractListViewItems(listViewBlackList),
				Occurrences = _filters.Occurrences,
