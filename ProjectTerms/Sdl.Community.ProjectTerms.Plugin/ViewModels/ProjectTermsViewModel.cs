...
namespace Sdl.Community.ProjectTerms.Plugin
{
	public class ProjectTermsViewModel
	{
		...
		private void ExtractProjectTerms()
		{
			...
			var projectPath = Utils.Utils.GetProjectPath();
		...
		{
			Terms = cache.ReadXmlFile(Utils.Utils.GetXMLFilePath(Utils.Utils.GetProjectPath(), true));
		}
		...
		{
			FilterProjectTerms();
			cache.Save(Utils.Utils.GetProjectPath(), Terms);
		}
		...
		public void AddXMlFileToProject()
		{
			...
			var xmlFolder = Path.GetDirectoryName(Utils.Utils.GetXMLFilePath(Utils.Utils.GetProjectPath()));
