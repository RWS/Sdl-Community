...
namespace Multilingual.Excel.FileType.FileType.Preview
{
	public class InternalPreviewFileTweaker : AbstractFilePostTweaker, INativeOutputSettingsAware
	{
		...
		public override void TweakFilePostWriting(INativeOutputFileProperties outputFileProperties)
		{
			...
			var regexSegment = new Regex(@"(?<fspan>(|\<SPAN\s+[^\>]+>\s*))&lt;" + FiletypeConstants.MultilingualSegment + @"\s+pid=""(?<pid>[^""]+)""\s+sid=""(?<sid>[^""]+)""&gt;(?<content>(|.*?))&lt;/" + FiletypeConstants.MultilingualSegment + @"&gt;(?<espan>(\s*</SPAN>|))", RegexOptions.Singleline);
			content = regexSegment.Replace(content, ReplaceSegment);

			var regexParagraphUnit = new Regex(@"(?<fspan>(|\<SPAN\s+[^\>]+>\s*))&lt;" + FiletypeConstants.MultilingualParagraphUnit + @"\s+pid=""(?<pid>[^""]+)""&gt;(?<content>(|.*?))&lt;/" + FiletypeConstants.MultilingualParagraphUnit + @"&gt;(?<espan>(\s*</SPAN>|))", RegexOptions.Singleline);
