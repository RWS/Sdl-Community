...
namespace TradosPluginTests
{
    ...
    public class SegmentParserTests
    {
        ...
        {
            Segment[] segments = SegmentParser.Parser.ParseFile(
                Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "NonexistentFile.txt"));
        }
        ...
        public void ParseLine_NoTags_ParsesText()
        {
            ...
            Segment segment = SegmentParser.Parser.ParseLine(text);
        ...
        {
            const string text =
                "Tick the box to accept <1 id=217>Terms and Conditions and click <2 id=220>Continue</2>.";
            Segment segment = SegmentParser.Parser.ParseLine(text);
        ...
        {
            const string text =
                "Tick the box to accept Terms and Conditions</1> and click <2 id=220>Continue</2>.";
            Segment segment = SegmentParser.Parser.ParseLine(text);
        ...
        public void ParseLine_ContainsNestedTag_CreatesOneTag()
        {
            ...
            Assert.AreEqual(@"<code class=""prettyprint"">master-host</code>",
                ((Tag)segment.Elements[1]).TextEquivalent);
            ...
            segment = SegmentParser.Parser.ParseLine(nestedQuotes);
            Assert.IsTrue(segment.Elements[1] is Tag);
        ...
        public void ParseTag_NoTag_Null()
        {
            const string text =
                @"Once downloaded, open the .sdlplugin file and the SDL Trados Studio plugin installer will launch.";
            Assert.IsNull(SegmentParser.Parser.ParseTag(text));
        }
        ...
        public void ParseTag_AbnormalID_IDCorrectlyParsed()
        {
            ...
            Assert.AreEqual(basicTagID, SegmentParser.Parser.ParseTag(string.Format(startingTagTemplate, basicTagID)).TagID);
            Assert.AreEqual(basicTagID, SegmentParser.Parser.ParseTag(string.Format(standaloneTagTemplate, basicTagID)).TagID);
            Assert.AreEqual(basicTagID, SegmentParser.Parser.ParseTag(string.Format(placeholderTagTemplate, basicTagID, "cat")).TagID);

            const string intricateTagID = @"abc123,.;'[]\-={}|:""<?";
            Assert.AreEqual(intricateTagID, SegmentParser.Parser.ParseTag(string.Format(startingTagTemplate, intricateTagID)).TagID);
            Assert.AreEqual(intricateTagID, SegmentParser.Parser.ParseTag(string.Format(standaloneTagTemplate, intricateTagID)).TagID);
            Assert.AreEqual(intricateTagID, SegmentParser.Parser.ParseTag(string.Format(placeholderTagTemplate, intricateTagID, "cat")).TagID);
        }
        ...
        public void ParseTag_XAttribute_IDCorrectlyParsed()
        {
            ...
            Assert.AreEqual(id, SegmentParser.Parser.ParseTag(startingTagWithXAttr).TagID);
            Assert.AreEqual(id, SegmentParser.Parser.ParseTag(standaloneTagWithXAttr).TagID);
            Assert.AreEqual(id, SegmentParser.Parser.ParseTag(placeholderTagWithXAttr).TagID);
        }
        ...
        public void ParseTag_IllegalID_NullTag()
        {
            ...
            Assert.IsNull(SegmentParser.Parser.ParseTag(string.Format(startingTagTemplate, idWithSpace)));
            Assert.IsNull(SegmentParser.Parser.ParseTag(string.Format(standaloneTagTemplate, idWithSpace)));
            Assert.IsNull(SegmentParser.Parser.ParseTag(string.Format(placeholderTagTemplate, idWithSpace, "cat")));

            const string idWithSlash = @"abc/123";
            Assert.IsNull(SegmentParser.Parser.ParseTag(string.Format(startingTagTemplate, idWithSlash)));
            Assert.IsNull(SegmentParser.Parser.ParseTag(string.Format(standaloneTagTemplate, idWithSlash)));
            Assert.IsNull(SegmentParser.Parser.ParseTag(string.Format(placeholderTagTemplate, idWithSlash, "cat")));
        ...
        {
            Assert.AreEqual(TagType.Start, SegmentParser.Parser.ParseTag(
                string.Format(startingTagTemplate, "123")).Type);
            Assert.AreEqual(TagType.End, SegmentParser.Parser.ParseTag("</1>").Type);
            Assert.AreEqual(TagType.Standalone, SegmentParser.Parser.ParseTag(
                string.Format(standaloneTagTemplate, "123")).Type);
            Assert.AreEqual(TagType.LockedContent, SegmentParser.Parser.ParseTag(
                string.Format(placeholderTagTemplate, "123", @"""<code class=""prettyprint"">8001</code>""/>")).Type);
        }
    }
}
