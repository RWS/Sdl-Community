...
namespace Sdl.Community.Comparison
{
	public class TextComparer
	{
		...
		internal class Merger
		{
			...
			internal int WordsInOriginalFile => _original.Count;


			internal int WordsInModifiedFile => _modified.Count;
			...
			private MiddleSnake FindMiddleSnake(Sequence src, Sequence des)
			{
				...
				for (d = 1; ; d++)
				{
					...
					for (k = fwdMax; k >= fwdMin; k -= 2)
					{
						...
						while (x < src.EndIndex && y < des.EndIndex &&
		...
		{
			var sourceLen = GetTotalCharCount(source, out var sourceItems, includeTags);
			var targetLen = GetTotalCharCount(target, out var targetItems, includeTags);
