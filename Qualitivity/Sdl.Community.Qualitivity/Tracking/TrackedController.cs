        ...
        public static void TrackActiveChanges(TrackedDocuments trackedDocuments)
        {
            try
            {
                    ...
                    != trackedDocuments.ActiveSegment.CurrentISegmentPairProperties.ConfirmationLevel.ToString() || trackedDocuments.ActiveSegment.CurrentSegmentContentHasChanged || targetCommentsChanged || qualityMetricChanged)
                {
                    ...
                    try
                    {
                        // attempts to grab the statistics associted with the document if it exist;
                        // to avoid inconsistency here, decided to avoid this for now... needs more research

                        //var segmentPairs = Tracked.ActiveDoc.GetSegmentPairsFromParagraph(trackedDocuments.ActiveSegment.CurrentSegmentSelected.GetParagraphUnitProperties().ParagraphUnitId);
                        //if (segmentPairs.Count() == 1)
                        //{
                        //var paragraphProperties = trackedDocuments.ActiveSegment.CurrentSegmentSelected.GetParagraphUnitProperties();

                        //grab the word count if it exists
                        //record.WordCount = (paragraphProperties.SourceCount != null
                        //    ? paragraphProperties.SourceCount.Value : 0);
                        //}
                    }
                    catch
                    {
                    ...
                    #region  |  getStatisticalContentCounts  |
