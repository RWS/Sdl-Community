...
namespace Sdl.Community.SdlDataProtectionSuite.SdlTmAnonymizer.ViewModel
{
    public sealed class ContentFilteringRulesViewModel : ViewModelBase, IDisposable
    {
        ...
        public ContentFilteringRulesViewModel(TranslationMemoryViewModel model, ExcelImportExportService excelImportExportService, GroupshareCredentialManager groupshareCredentialManager)
        {
            ...
            if (selectedTms.Count > 0 && selectedRulesCount > 0)
            {
                ...
                if (serverTms.Any())
                {
                    ...
                    foreach (var serverTm in serverTms)
                    {
                        ...
                        if (providers.Count(a => a.Uri == uri) != 0) continue;

                        var provider =
                            GroupshareCredentialManager.TryGetProviderWithoutUserInput(serverTm.Credentials);
