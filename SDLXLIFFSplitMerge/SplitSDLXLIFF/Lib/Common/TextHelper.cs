...
using Sdl.Community.Toolkit.ProjectAutomation;
...
namespace Sdl.Utilities.SplitSDLXLIFF.Lib
{
	...
	public static class TextHelper
    {
        ...
        {
		//	var usedAssemblies = LoadAssemblies();
	        ...
	        var studioLocation = ExecutingStudioLocation();


			// make setups from Sdl.LanguagePlatform.TranslationMemoryTools.dll
			//get assembly
	        if (studioLocation.Contains(@"Program Files (x86)\SDL\SDL Trados Studio"))
	        {
				// used in Trados Studio
				translationMemoryToolsAssembly = Assembly.LoadFrom(Path.Combine(studioLocation, "Sdl.LanguagePlatform.TranslationMemoryTools.dll"));
				//translationMemoryToolsAssembly = usedAssemblies["Sdl.LanguagePlatform.TranslationMemoryTools"];

			}
			else
	        {
				// used in the separate app (Sdl.Community.SplitMergeUIWix) generated by the Wix installer
				  translationMemoryToolsAssembly = Assembly.LoadFrom(Path.Combine(@"C:\Program Files (x86)\SDL\SDL Trados Studio\Studio5", "Sdl.LanguagePlatform.TranslationMemoryTools.dll"));
				//translationMemoryToolsAssembly = usedAssemblies["Sdl.LanguagePlatform.TranslationMemoryTools"];
			}
	        ...
	        {
				// used in Trados Studio
				 languageProcessingAssembly = Assembly.LoadFrom(Path.Combine(ExecutingStudioLocation(), "Sdl.Core.LanguageProcessing.dll"));
				//languageProcessingAssembly = usedAssemblies["Sdl.Core.LanguageProcessing"];

			}
			else
	        {
				// used in the separate app (Sdl.Community.SplitMergeUIWix) generated by the Wix installer
				languageProcessingAssembly = Assembly.LoadFrom(Path.Combine(@"C:\Program Files (x86)\SDL\SDL Trados Studio\Studio5", "Sdl.Core.LanguageProcessing.dll"));
				//languageProcessingAssembly = usedAssemblies["Sdl.Core.LanguageProcessing"];
			}
		...
		//		return null;
		//	}
		//	var appAssembly = typeof(TextHelper).Assembly;
